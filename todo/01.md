...
- arch diagrams for each service/set: dependencies, logging, monitoring, health checks, resillicency patterns
    - detail into ADP.mds
...
- deploy ws
    - kubernetes in action / etc; First pass; videos; curric - current/kube
    - try play with docker, start trying cluster setup & deploy
    - cloud patterns curric (list of tech, path for myself niche long lasting)
- docker sales vids

- Docker study group, vids; sandbox work for ramp up - base images, increment on sandbox/workshop
    - make outline
