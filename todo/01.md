...
### Today/Near Timely
*R* Regs above (GO THRU FULLY SOMETIME) ::gaps, career tweak (no weeds of code but tech depth of cloud arch, optimization & dev efficiency?)
*I*!!! WORKSHOPS increment, promote; scala/react/web guts/PDP [I]

budg
fadi quote, assessment, Jeff devops simulation, SM training agile
docker ws tb
    - save 2 seats
    - break sections into md files. Add intro/overview to each section
        - add images
    - practice talking, insert intersting things (update grah numbers)
    - mention orchestion in more detail
    - mention docker enterprise with others, have slide comparing
    - update versions
pd spreadsheet
    - mark/add core competencies (diff list?); add brown bag series around core competencies


[02]docker Dylan stuff; who else?
list probs - set up meeting for strategy

leadership training
pd cleanup, update, try with reports
docker deploy ws outline; increment

code camp & cmap topic
...
figure out tomorrow (ivy)

practice belay

minimum preparation for dist programming on SAVE
